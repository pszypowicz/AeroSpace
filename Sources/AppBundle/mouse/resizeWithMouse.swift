...
/// borrow the dimension from any descendant leaf.
///
/// Borrowing relies on the alternation invariant: in a properly alternating
/// tree, a container's children split perpendicular to the parent, so all
/// descendants of a container share the parent-orientation extent. The
/// existing `enable-normalization-opposite-orientation-for-nested-containers`
/// pass enforces this. With that normalization disabled, a same-orientation
/// nested container can leave the borrowed dimension shorter than the
/// container's actual extent, and the reconcile underestimates that
/// container's weight. The next layout pass corrects sibling proportions, so
/// the visible effect is at most a one-frame jitter rather than a permanent
/// drift.
@MainActor
