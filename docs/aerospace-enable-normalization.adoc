...
`<kind>` is one of `flatten-containers`, `opposite-orientation-for-nested-containers`, `bsp-shape` (the suffix of the corresponding `enable-normalization-<kebab-name>` global config key). See xref:bsp-mode.adoc[the BSP mode guide] for the recommended `bsp-shape` workflow.
