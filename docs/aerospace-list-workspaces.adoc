...
%{workspace-is-hollow}:: Boolean. `true` if the workspace holds macOS native fullscreen windows (each on its own macOS Space) but nothing on its host desktop, so visiting it shows a bare desktop
