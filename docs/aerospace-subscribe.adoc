...
focus-changed:: Fired when window focus changes. Includes `windowId`, `workspace`, `macosFullscreen`.
`macosFullscreen` is `true` when the focused window is in macOS native fullscreen (its own macOS Space),
as opposed to AeroSpace's own `fullscreen` mode.
...
{"_event":"focus-changed","macosFullscreen":false,"windowId":28218,"workspace":"M"}
----
